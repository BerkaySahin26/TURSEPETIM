    ...
    public class VisitorController : ControllerBase
    {
        ...
        public IActionResult CreateVisitor()
        {
            ...
            Enumerable.Range(1, 100000).ToList().ForEach(x =>
            {
                ...
                {
                    var newVisitor = new Visitor  // Yeni bir Visitor nesnesi oluşturduk.
                    {
                        City = item,
                        CityVisitCount = random.Next(100, 2000),
                        VisitDate = DateTime.Now.AddDays(x)
                    };
                    _visitorService.SaveVisitor(newVisitor).Wait(); // VisitorService üzerinden yeni ziyaretçi kaydettik (asenkron olarak)
