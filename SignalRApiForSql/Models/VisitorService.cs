...
namespace SignalRApiForSql.Models
{
    public class VisitorService
    {
        ...
        public IQueryable<Visitor> GetList() // Veritabanından ziyaretçi listesini sorgulayarak IQueryable olarak döndüren metot.
        {
        ...
        {
            await _context.Visitors.AddAsync(visitor); // Yeni ziyaretçiyi veritabanına ekleme.
            await _context.SaveChangesAsync();
            await _hubContext.Clients.All.SendAsync("ReceiveVisitorList", GetVisitorChartList());
        }
        public List<VisitorChart> GetVisitorChartList()
        {
            List<VisitorChart> visitorCharts = new List<VisitorChart>(); // Ziyaretçi grafik verilerini burada tut
            using (var command = _context.Database.GetDbConnection().CreateCommand())
            {
                ...
                {
                    while (reader.Read())
                    {  // Şehirlerin ziyaret sayıları, eğer veri yoksa 0 olarak ekleniyor
                        ...
                        visitorCharts.Add(visitorChart); // Oluşturulan VisitorChart nesnesi listeye ekle.
                    }
                }
